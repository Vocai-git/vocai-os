...
<div class="chats-conv-head">
  ...
  <button class="chats-head-icon-btn" title="Borrar conversación (empezar de cero)" onclick="chatsBorrarConv('${id}')" style="margin-right:4px;color:#ef4444">
...
async function chatsBorrarConv(id){
  if (!confirm('¿Borrar toda esta conversación?\n\nSe eliminan todos los mensajes para empezar de cero. No se puede deshacer.')) return;
  try {
    await API.del(`/tito/chats/${id}`);
    chatConvActiva = null;
    const emptyEl = document.getElementById('chatsPanelEmpty');
    const convEl  = document.getElementById('chatsPanelConv');
...
window.chatsBorrarConv    = chatsBorrarConv;
